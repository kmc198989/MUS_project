						...
						<div class="right">
								...
								<ul class="productul">
									...
									<li class="proli">
										...
										<div class="prodiv2">
											...
											</span> <span class="ratingavg">${goodsInfo.ratingAvg}</span> <span
												class="ratingcount"> 후기 ${goodsInfo.ratingCount}개 </span>
										</div>
									</li>
									<hr>
									<li class="proli top">
									...
									<li class="proli">
										...
										<div class="price-info info prodiv2">
											<span class="discount_price_number"><fmt:formatNumber
													...
													value="${goodsInfo.clothDiscount*100}" pattern="###" /> %
												<fmt:formatNumber
							...
							<div class="button info top">
								...
								<div class="button_set">
									...
									<span class="likesnum">${goodsInfo.likes}</span>
									</a>
								</div>
							</div>
...
<script>
	$(document)
            ...
            const likeButton = $('.likeicon');
            const likesSpan = $('.likesnum');
            const currentLikes = parseInt(likesSpan.text(), 10);
						...
						$.ajax({
							...
							url : '/likes/check',
							type : 'POST',
				      dataType: 'json', // 명시적으로 응답 타입을 json으로 설정
							success : function(result) {
				
								if (result === 1) {
                  likeButton.removeClass('unlike').addClass('like');
								} else if (result === 0) {
	...
	document.querySelector('.likeicon').addEventListener('click', function() {
	    const likesSpan = document.querySelector('.likesnum');
      ...
      dataType: 'json', // 명시적으로 응답 타입을 json으로 설정
           ...
           $.ajax({
               ...
               success: function(deleteResult) {
                   console.log("좋아요 삭제 성공: ", deleteResult);
               },
               error: function(xhr, status, error) {
                   console.error("좋아요 삭제 실패: ", status, error);
               }
           });
          ...
          $.ajax({
              ...
              success: function(addResult) {
                  console.log("좋아요 추가 성공: ", addResult);
              },
              error: function(xhr, status, error) {
                  console.error("좋아요 추가 실패: ", status, error);
              }
          });
